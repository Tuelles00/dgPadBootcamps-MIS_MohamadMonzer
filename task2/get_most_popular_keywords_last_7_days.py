...
def get_most_popular_keywords_last_7_days():
    ...
    latest_doc = collection.find_one(sort=[("published_time", -1)])
    if not latest_doc:
        print("No documents found in the collection.")
        return

    latest_date_str = latest_doc.get('published_time')
    if not latest_date_str:
        print("No 'published_time' field found in the most recent document.")
        return
